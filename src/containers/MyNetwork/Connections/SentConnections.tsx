...
const SentConnections: React.FC = () => {
  ...
  const {
    ...
    `GET- ${API_ENDPOINTS.USER_CONNECTIONS.key}:${authUser?.id}:SENT:${remountKey}`,
    () =>
      getUserConnections(
      ...
      <Grid container p={1} pb={2} spacing={1}>
        {getUserConnectionsStatus === "success" &&
          upToDateUserConnections.length <= 0 && (
            <Grid item xs={12}>
              <Typography>{LABELS.CONNECTION_NO_SENT_CONNECTIONS}</Typography>
            </Grid>
          )}
